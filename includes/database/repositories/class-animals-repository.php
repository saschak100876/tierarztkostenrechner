...
class TKR_Animals_Repository {

    private string $table;

    public function __construct() {
        global $wpdb;
        $this->table = $wpdb->prefix . 'tkr_animals';
    }

    public function get_all_active(): array {
        global $wpdb;
        $cache_key = 'tkr_animals_active';
    ...
    public function get_by_uid( string $uid ): ?array {
        global $wpdb;
        $row = $wpdb->get_row(
            $wpdb->prepare( "SELECT * FROM `{$this->table}` WHERE animal_uid = %s LIMIT 1", $uid ),
            ARRAY_A
        );
        return $row ?: null;
    }
